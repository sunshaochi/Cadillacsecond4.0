...
/**
 * Created by bitch-1 on 2017/7/7.
 */

public class CopyLineView extends View implements ValueAnimator.AnimatorUpdateListener {
    // data
     ...
     *            x轴的坐标值
     */
    public CopyLineView(List<List<String>> dataSource, String[] yData, String[] xData, Context context) {
        super(context);
        this.dataSource = dataSource;
        this.yData = yData;//y轴坐标
        this.xData = xData;//x轴坐标
        // 初始化颜色数组
        LINE_COLOR_DATA = new String[dataSource.size()];//线的颜色
        TEXT_COLOR_DATA = new String[dataSource.size()];//字体的颜色
        // 初始化画笔
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 开始画线
        // 画x轴
        canvas.drawLine(xAxisStart, yStart, xStart + (width - 2 * MARGIN_SIZE_HORIZONTAL), yStart, paintLineX);
        // 写x轴文字
...
//            if (i % 2 == 0) {
                ...
                canvas.drawTextOnPath(xData[i], path, 0, 0, paintTextX);
...
//        }
        // 写y轴文字
         for (int i = 0; i < yData.length; i++) {
             float tempWidth = paintTextX.measureText(yData[i]);
             canvas.drawText(yData[i], xAxisStart - tempWidth - 15, yStart - i * SCALE_Y + TEXT_SIZE_Y / 2, paintTextY);
             if(i >0){
                canvas.drawLine(xAxisStart, yStart - i * SCALE_Y, xStart + (width - 2 * MARGIN_SIZE_HORIZONTAL),
                        yStart - i * SCALE_Y, paintLineX);
             }
         }
//        if (Float.parseFloat(yData[1]) == 0) {// 如果最大值和最小值都为0,那么应该做特殊处理.将0的位置设为y轴中间
//            float tempWidth = paintTextX.measureText(yData[1]);
//            canvas.drawText(yData[1], xAxisStart - tempWidth - 15, yStart - 2 * SCALE_Y + TEXT_SIZE_Y / 2, paintTextY);
//        } else {
//            // 最小值
//            float tempWidth = paintTextX.measureText(yData[0]);
//            canvas.drawText(yData[0], xAxisStart - tempWidth - 15, yStart - SCALE_Y / 2 + TEXT_SIZE_Y / 2, paintTextY);
//            // 最大值
...
//            canvas.drawText(tempMaxY, xAxisStart - tempWidth1 - 15, yStart - 7 * SCALE_Y / 2 + TEXT_SIZE_Y / 2, paintTextY);
//            // 中间值
...
//                canvas.drawText(tempMidY, xAxisStart - tempWidth2 - 15, yStart - 2 * SCALE_Y + TEXT_SIZE_Y / 2, paintTextY);
//            }
//        }
...
//                switch (i) {
//                    case 0:
                if(Math.abs(Double.parseDouble(stockList.get(i).getyData())-Double.parseDouble(yData[0]))<30){
                    canvas.drawText(stockList.get(i).getyData(), stockList.get(i).getPosition().x + 5,
                            stockList.get(i).getPosition().y, paintTextData[i]);
                }else{
                    canvas.drawText(stockList.get(i).getyData(), stockList.get(i).getPosition().x + 5,
...
//                }
    ...
    void init() {
        ...
        height = getMeasuredHeight();//控件高度
        // 计算xStart,yStart
        xStart = MARGIN_SIZE_HORIZONTAL + xTextSkew;//x轴开始位置
        xAxisStart = MARGIN_SIZE_HORIZONTAL;
        yStart = MARGIN_SIZE_VERTICAL + (height - 2 * MARGIN_SIZE_VERTICAL);//y轴开始位置

        MARGIN_SIZE_HORIZONTAL = width/10;//y周距离左边
        MARGIN_SIZE_VERTICAL = height/6;//x周距离下边
        // 计算SCALE_X SCALE_Y
        SCALE_X = (width -2 * MARGIN_SIZE_HORIZONTAL - xTextSkew) / (xData.length - 1);//x刻度长度
        SCALE_Y = (height - 2* MARGIN_SIZE_VERTICAL) / (yData.length-1);//y刻度长度
        // 初始化颜色 可以在此处进行颜色的更改
        LINE_COLOR_X = "#EEEEEE";
        LINE_COLOR_Y = "#333333";
        TEXT_COLOR_X = "#333333";
...
//        if (TEXT_COLOR_DATA.length == 2) {
            TEXT_COLOR_DATA[1] = "#FF00FF";
            LINE_COLOR_DATA[1] = "#FF00FF";
//        }
        // 初始化文字大小
...
//                if (Float.parseFloat(yData[]) == 0) {
//                    // 如果最大值和最小值都为0,那么应该做特殊处理.将0的位置设为y轴中间,同时所有的数据都为0
//                    y = (float) (yStart - 2 * SCALE_Y);
//                    tempLineViewPointBean.setyData("0");
//                } else {
                    y = yStart - (yData.length-1)*((Float.parseFloat(tempDataSource.get(j)) - Float.parseFloat(yData[0]))
                            / (Float.parseFloat(yData[yData.length-1]) - Float.parseFloat(yData[0]))) * SCALE_Y;
//                }
                temPointF.set(x, y);
