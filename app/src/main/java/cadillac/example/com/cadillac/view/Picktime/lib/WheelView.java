package cadillac.example.com.cadillac.view.Picktime.lib;
...
import cadillac.example.com.cadillac.view.Picktime.adapter.WheelAdapter;
...
public class WheelView extends View {
	...
	private void measureTextWidthHeight() {
		...
		for (int i = 0; i < adapter.getItemsCount(); i++) {
			...
			paintCenterText.getTextBounds("\u661F\u671F", 0, 2, rect); // 星期(中文一般比数字要大,所以用中文测量最大高度)
	...
	private void measuredCenterContentStart(String content) {
		...
		switch (mGravity) {
		case Gravity.CENTER:
			drawCenterContentStart = (int) ((((measuredWidth - rect.width()) > 0 ? (measuredWidth - rect.width()) : 0))
					* 0.5);
			break;
		case Gravity.LEFT:
