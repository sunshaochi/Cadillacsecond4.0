...
//        this.UIDisplayer = UIDisplayer;
    ...
    public BreakpointUploadTask asyncUpload(String objectName, String localFileName) {

        final String object = new String(objectName);
        final String localFile = new String(localFileName);

        final BreakpointUploadRequest request = new BreakpointUploadRequest(bucket, object, localFile, partSize);
        request.setProgressCallback(new OSSProgressCallback<BreakpointUploadRequest>() {
            //上传回调，这里的进度是指整个分片上传的进度
            @Override
            public void onProgress(BreakpointUploadRequest breakpointUploadRequest, long currentSize, long totalSize) {
...
//                UIDisplayer.displayInfo("上传进度: " + String.valueOf(progress) + "%");
        ...
        final BreakpointUploadTask task = new BreakpointUploadTask(oss, request, new OSSCompletedCallback<BreakpointUploadRequest, BreakPointUploadResult>() {
            //上传成功
            @Override
            public void onSuccess(BreakpointUploadRequest request, BreakPointUploadResult result) {
...
//                UIDisplayer.displayInfo("Bucket: " + bucket + "\nObject: " + request.getObject() + "\nETag: " + result.getETag() +  "\nRequestId: " + result.getRequestId());
            ...
            //上传失败
            @Override
            public void onFailure(BreakpointUploadRequest request, ClientException clientExcepion, ServiceException serviceException) {
                String info = "";
                // 请求异常
